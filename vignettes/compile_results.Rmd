...
fct_dat <- final_results %>%
  dplyr::mutate(
    .data = .,
    mod = forcats::as_factor(mod),# `1` = "HLM", `2` = "MMREM"),
    form = forcats::as_factor(form),
    pctmob = forcats::as_factor(pctmob),
    cor = forcats::as_factor(cor),
    icc = forcats::as_factor(icc)
...
c("fp_int", "fp_x1", "rp_int_v1", "rp_int_v2") %>%
  purrr::map2(.,
    ...
    .f = ~{
      ...
      # save plot
      ggplot2::ggsave(
        file.path(path, paste0("avgse_wrap_", .y, ".png"))
      )
    })
...
c("fp_int", "fp_x1", "rp_int_v1", "rp_int_v2") %>%
  purrr::map2(
    ...
    .f = ~{
        ...
        ggplot2::ylab("Mean Standard Error") +
        ggplot2::ggtitle(.y)
      
      # save plot
      ggplot2::ggsave(
        file.path(path, paste0("mse_indiv_", .y, ".png"))
      )
    })
  ...
  purrr::map2(.,
    ...
    .f = ~{
      ...
      temp %>%
        ...
        ggplot2::scale_x_discrete(
          name = "Model Specification",
          labels = c("Null", "L2 Covariate", "L1 & L2 Covariate")
        ) +
        ggplot2::scale_shape_discrete(
         name = "Cluster Correlation"
        ) +
        ...
        ggplot2::ylab("Mean Square Error") +
        ggplot2::ggtitle(.y)

      # save plot
      ggplot2::ggsave(
        file.path(path, paste0("mse_form_", .y, ".png"))
      )
    })
